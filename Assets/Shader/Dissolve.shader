    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                float4 _DissolvePattern_var = tex2D(_DissolvePattern,TRANSFORM_TEX(i.uv0, _DissolvePattern)); // dissolve pattern
                float node_7856 = (_DissolvePattern_var.r+(_ClipAmount*1.4+-0.6));
                clip(node_7856 - 0.5);
////// Lighting:
////// Emissive:
                float4 _Sprite_var = tex2D(_Sprite,TRANSFORM_TEX(i.uv0, _Sprite));
                float node_6010 = (1.0 - saturate((node_7856*8.0+-4.0)));
                float2 node_7684 = float2(node_6010,0.0);
